...
function toTab(content,firstlevel,itemName,parents,color){
  ...
  var isCollection = (content.constructor === Array);
	...
	if(!firstlevel){
  	...
  	$(superWrapper).click( function(event){
  	  if(['DIV', 'TABLE', 'TD'].indexOf(event.target.tagName) +1){
  ...
  navTable.className = 'navTab';
	...
	if(table.rows.length > 0 || navTable.rows.length > 0 || keyTable.rows.length > 0){
  	...
  	if(content.constructor !== Array){
    	...
    	if(keyTable.rows.length > 0){
    	  ...
    	  var miscTab  = document.createElement('table');
    	  var miscRow  = miscTab.insertRow();
    	  var miscCell = miscRow.insertCell();
    	  miscTab.className = 'miscTab';
    	  
    	  entityCell.appendChild(miscTab);
      	...
      	miscCell.appendChild(collectionLink);
      	
      	if(!firstlevel){
    	    var check       = document.createElement('input');
    	    check.type      = 'checkbox';
    	    check.className = 'useForCreate';
    	    $(check).hide();
    	    
    	    var label       = document.createElement('label');
    	    label.className = 'useForCreateLabel';
    	    label.innerHTML = 'send';
    	    $(label).hide();
    	    
      	  miscCell = miscRow.insertCell();
    	    miscCell.appendChild(check);
    	    miscCell.appendChild(label);
      	}
    	}
	...
	// ==============================================================
  	...
  	propWrapper.className = 'propWrapper';
  	var propWrapper2 = document.createElement('div');
  	propWrapper2.className = 'propWrapper2';
    ...
    //}
  	...
  	if( typeof value != 'object'){
        ...
        value       = date.getFullYear()+'-'+("0"+(date.getMonth()+1)).slice(-2)+'-'+("0"+date.getDate()).slice(-2)+'T'+
  		...
  		var check         = document.createElement('input');
  		check.type        = 'checkbox';
  		check.className   = 'usePropForCreate';
  		check.checked     = true;
